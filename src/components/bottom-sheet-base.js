...
export default class BottomSheetBase extends HTMLElement {
    ...
    _bindOverlay() {
        if (this._offOverlay) this._offOverlay();
        this._offOverlay = addPassiveListener(
            this._overlay,
            "click",
            () => this.closeSheet(),
            { passive: true }
    ...
    _bindDrag() {
        ...
        try {
            this._dragController = createDragController(this._sheetWrapper, {
                ...
                useTransform: true,
                onStart: () => {
                    this._sheetWrapper.classList.add("not-selectable");
                },
                onMove: info => {
    ...
    _destroyDrag() {
        if (
            ...
            typeof this._dragController.destroy === "function"
        ) {
    ...
    openSheet() {
        if (this.defaultVh === 0) {
            if (this.hasAttribute("vh"))
                this.defaultVh = Number(this.getAttribute("vh"));
            else
                this.defaultVh =
                    (this._sheetWrapper.offsetHeight / window.innerHeight) *
                        100 || 50;
        }
        ...
        nextFrame().then(() => {
            try {
                this._sheetWrapper.focus && this._sheetWrapper.focus();
            } catch (_) {}
        });
    }
