...
export async function createSanitizer(
    ...
    if (!globalWindow)
        throw new Error(
            "createSanitizer requires a window-like global (browser environment)."
        );

    // If DOMPurify already present globally (e.g., loaded via <script>), use it
    if (
        globalWindow.DOMPurify &&
    ...
    try {
        ...
        throw new Error(
            'DOMPurify not available. Install "dompurify" or provide your own sanitizeFn.'
        );
    }
}
 ...
 */
export function createSanitizerSync(
    globalWindow = typeof window !== "undefined" ? window : undefined
) {
